...
if game.GetMap() == "fnap_scc" then resource.AddWorkshop("542724054") end
	...
	if gm.Vars.iniok and gm.Vars.mapoverrideok and gm.Vars.startday and gm.Vars.active and map == "fnap_scc" then
		...
		if gm.Vars.power <= 0 and not gm.Vars.poweroff then
			...
			if #legacy_ent > 0 then
				...
				if IsValid(legacy_ent) then legacy_ent:Fire("use") end
			end

			for _, v in pairs(player.GetAll()) do
				if v:Team() == 1 and v:Alive() then v:SetPos(Vector(-465, -255, 32)) end
			end

			for k, _ in pairs(gm.Vars.Animatronics) do
				if k ~= gm.Animatronic.RainbowDash then gm:SetAnimatronicPos(nil, k, gm.APos[map].SS) end
...
function fnapgmRainbowDash(self)
	if self:GetAType() == GAMEMODE.Animatronic.RainbowDash then
		if self.FoxyMove then
			...
			self.FoxyMove2 = true
			for _, v in pairs(player.GetAll()) do
				if v:Team() ~= TEAM_CONNECTING and v:Team() ~= TEAM_UNASSIGNED then v:SendLua([[LocalPlayer():EmitSound("fnapgm_runrainbowdash")]]) end
			end
		end
...
function fnapgmGoJumpscare(me, ent, timet)
	local gm = GAMEMODE
	local map = game.GetMap()
	if map == "fnap_scc" then
		...
		timer.Create("fnafgmJumpscare" .. me, timet, 1, function()
			...
			elseif gm.Vars.startday then
				...
				timer.Create("fnapgmRDmove", timet, 1, function()
					...
					ent.FoxyWillMove = false
					ent.FoxyWillMove2 = false
					ent.FoxyMove = true
...
function fnapgmJumpscare(me, ent)
	local gm = GAMEMODE
	local map = game.GetMap()
	if map == "fnap_scc" then
		...
		if me == gm.Animatronic.RainbowDash then
			timer.Create("fnafgmFoxyReset", 1, 1, function()
				ent:SetPos(gm.AnimatronicAPos[me][map][gm.APos[map].Trash][1])
				ent:SetAngles(gm.AnimatronicAPos[me][map][gm.APos[map].Trash][2])
